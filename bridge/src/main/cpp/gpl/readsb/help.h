...
static struct argp_option optionsReadsb[] = {
    ...
    {"auto-exit", OptAutoExit, "<sec>", 0, "Run for X seconds, then exit (default: run indefinitely)", 1},
    ...
    {"write-prom", OptPromFile, "<file>", 0, "Periodically write prometheus output to <file>", 1},
    ...
    {0,0,0,0, "Network options:", 2},
    {"net-connector", OptNetConnector, "<IP,PORT,PROTOCOL>", 0, "connect as TCP client to listen port / TCP server at IP and PORT, can be specified multiple times (e.g. 127.0.0.1,23004,beast_out) Protocols: beast_out, beast_in, raw_out, raw_in, sbs_in, sbs_in_jaero, sbs_out, sbs_out_jaero, vrs_out, json_out, gpsd_in, uat_in, uat_replay_out, planefinder_in, asterix_in, asterix_out (one failover ip/address,port can be specified: primary-address,primary-port,protocol,failover-address,failover-port) (any position in the comma separated list can also be either silent_fail or uuid=<uuid>)", 2},
    ...
    {"net-uat-replay-port", OptNetUatReplayPorts, "<ports>", 0, "UAT replay output listen port / TCP server (default: 0)", 2},
    {"net-uat-in-port", OptNetUatInPorts, "<ports>", 0, "UAT input listen port / TCP server (default: 0)", 2},
    {"net-sbs-port", OptNetSbsPorts, "<ports>", 0, "TCP BaseStation output listen port / TCP server (default: 0)", 2},
    {"net-sbs-in-port", OptNetSbsInPorts, "<ports>", 0, "TCP BaseStation input listen port / TCP server (default: 0)", 2},
    {"net-sbs-jaero-port", OptNetJaeroPorts, "<ports>", 0, "TCP SBS Jaero output listen port / TCP server (default: 0)", 2},
    {"net-sbs-jaero-in-port", OptNetJaeroInPorts, "<ports>", 0, "TCP SBS Jaero input listen port / TCP server (default: 0)", 2},
    {"net-asterix-out-port", OptNetAsterixOutPorts, "<ports>", 0, "TCP Asterix output listen port / TCP server (default: 0)", 2},
    {"net-asterix-in-port", OptNetAsterixInPorts, "<ports>", 0, "TCP Asterix input listen port / TCP server (default: 0)", 2},
    {"net-asterix-reduce", OptNetAsterixReduce, 0, 0, "Apply beast reduce logic and interval to ASTERIX outputs", 2},
    {"net-vrs-port", OptNetVRSPorts, "<ports>", 0, "TCP VRS json output listen port / TCP server (default: 0)", 2},
    {"net-vrs-interval", OptNetVRSInterval, "<seconds>", 0, "TCP VRS json output interval (default: 5.0)", 2},
    {"net-json-port", OptNetJsonPorts, "<ports>", 0, "TCP json position output listen port / TCP server, sends one line with a json object containing aircraft details for every position received (default: 0) (consider raising --net-ro-size to 8192 for less fragmentation if this is a concern)", 2},
    ...
    {"net-json-port-wind-triggered", OptNetJsonPortWindTriggered, 0, 0, "TCP json position output: instead of emitting when a new position is calculated, only emit when the wind was calculated now based on data from the last 2.5 seconds and the position is no older than 2.5 seconds", 2},
    {"net-api-port", OptNetApiPorts, "<port>", 0, "http API listen port (in contrast to other listeners, only a single port is allowed) (update frequency controlled by write-json-every parameter) (default: 0)", 2},
    ...
    {"beast-baudrate", OptBeastBaudrate, "<baud>", 0, "Override Baudrate (default rate 3000000 baud, try 1000000 / 921600 as alternatives)", 5},
