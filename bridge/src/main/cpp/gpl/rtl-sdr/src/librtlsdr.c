...
int rtlsdr_open_from_fd(rtlsdr_dev_t **out_dev, intptr_t file_descriptor)
{
	int r;
	uint8_t reg;
	...
	r = libusb_wrap_sys_device(dev->ctx, file_descriptor, &dev->devh);
	if (r < 0 || !dev->devh) {
		fprintf(stderr, "usb_wrap_sys_device error %d\n", r);
		goto err;
	}
